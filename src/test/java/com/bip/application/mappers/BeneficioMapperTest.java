...
class BeneficioMapperTest {
    ...
    @Nested
    @DisplayName("Update Entity (Deprecated Method)")
    @SuppressWarnings("removal")
    class UpdateEntityDeprecatedTests {
        
        @Test
        @DisplayName("Should update entity with DTO data")
        void shouldUpdateEntityWithDtoData() {
            // Given
            Beneficio entity = Beneficio.criar(
                "Nome Original",
                "Descrição Original",
            ...
            assertThat(entity.getDescricao()).isEqualTo("Descrição Atualizada");
            assertThat(entity.getSaldo().getValor()).isEqualByComparingTo(new BigDecimal("200.00"));
        }
        ...
        void shouldHandleNullDtoInUpdate() {
            ...
            String originalNome = entity.getNome();
            String originalDescricao = entity.getDescricao();
            BigDecimal originalValor = entity.getSaldo().getValor();
            
            // When
            mapper.updateEntity(entity, null);
            
            // Then - Should remain unchanged
            assertThat(entity.getNome()).isEqualTo(originalNome);
            assertThat(entity.getDescricao()).isEqualTo(originalDescricao);
            assertThat(entity.getSaldo().getValor()).isEqualByComparingTo(originalValor);
        }
        
        @Test
        @DisplayName("Should update entity with null valor in DTO")
        void shouldUpdateEntityWithNullValorInDto() {
            // Given
            Beneficio entity = Beneficio.criar(
                "Nome Original",
                "Descrição Original",
            ...
            assertThat(entity.getNome()).isEqualTo("Nome Atualizado");
            assertThat(entity.getDescricao()).isEqualTo("Descrição Atualizada");
            // Valor não deve ser alterado quando DTO tem valor null
            assertThat(entity.getSaldo().getValor()).isEqualByComparingTo(new BigDecimal("100.00"));
        }
    }
}